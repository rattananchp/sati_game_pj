...
export default function Users({ API_URL, currentUserRole }: UsersProps) {
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            if (json.users) {
                const sortedUsers = json.users.sort((a: User, b: User) => a.uid - b.uid);
    ...
    return (
        // ✅ เอา mt-10 ออก และใส่ overflow-hidden เพื่อล็อกหน้าจอไม่ให้เลื่อนทะลุกรอบ
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col relative overflow-hidden">

            {/* ส่วน Header และตัวกรอง (shrink-0 ป้องกันการโดนบีบ) */}
            <div className="shrink-0 flex flex-col xl:flex-row justify-between items-start xl:items-center mb-4 gap-4 bg-slate-900/50 p-4 rounded-2xl border border-white/5">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full xl:w-auto">
                    ...
                    <div className="relative group w-full sm:w-56">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-slate-800 border border-slate-700 rounded-xl text-sm text-gray-300 focus:outline-none focus:border-emerald-500 focus:ring-1 transition-all placeholder:text-gray-500"
                        />
                    </div>

                    {/* ✅ เพิ่มปุ่มเลื่อนหน้า (ขนาดเล็ก) มาไว้ข้างช่องค้นหา */}
                    <div className="flex bg-slate-800 rounded-xl border border-slate-700 overflow-hidden w-full sm:w-auto shrink-0">
                        <button onClick={handlePrevPage} disabled={page === 1} className="px-3 py-2 text-gray-400 hover:text-white hover:bg-white/5 disabled:opacity-30 transition flex justify-center">⬅️</button>
                        <span className="px-4 py-2 text-sm font-bold flex items-center justify-center border-l border-r border-slate-700 text-gray-300 min-w-[50px]">{page}/{totalPage}</span>
                        <button onClick={handleNextPage} disabled={page === totalPage} className="px-3 py-2 text-gray-400 hover:text-white hover:bg-white/5 disabled:opacity-30 transition flex justify-center">➡️</button>
                    </div>
                </div>
            </div>

            {/* ✅ ตารางแสดงข้อมูล (min-h-0 ช่วยล็อกไม่ให้ตารางขยายทะลุจอ) */}
            <div className="flex-1 min-h-0 bg-slate-900/80 rounded-2xl border border-slate-700 overflow-hidden shadow-xl flex flex-col backdrop-blur-md mb-2">
                <div className="overflow-auto flex-1 custom-scrollbar relative">
                    <table className="w-full text-left text-sm min-w-[850px]">
                        <thead className="bg-slate-950/90 text-gray-400 uppercase text-xs sticky top-0 z-10 backdrop-blur-xl border-b border-slate-800">
                            <tr>
                                <th className="p-4 w-[8%] text-center">ID</th>
                                <th className="p-4 w-[32%] pl-8">ผู้ใช้งาน</th> 
                                <th className="p-4 w-[25%]">ข้อมูลติดต่อ</th>
                                <th className="p-4 w-[20%] text-center">สถานะ (Role)</th>
                        ...
                        <tbody className="divide-y divide-slate-800/50">
                            {isLoading ? (
                                ...
                                if (currentUserRole === 'admin') {
                                    canManage = !isTargetAdmin; 
                                } else if (currentUserRole === 'editor') {
                                    canManage = isTargetUser;   
                                }
